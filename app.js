...
// Crop Box coordinates (normalized 0-1)
...
// Resize overlays to match containers
...
async function startWebcam() {
  try {
    ...
    trackingStatus.innerHTML = `<span class="status-dot red"></span><span class="status-text">Camera Denied</span>`;
    cameraPlaceholder.querySelector('p').textContent = 'Camera access denied. Please allow camera permissions.';
  }
}
  ...
  if (mode === 'calibrate') {
    if (results.multiHandLandmarks.length >= 2) {
      ...
      if (Math.abs(cropBox.x2 - cropBox.x1) > 0.15 && Math.abs(cropBox.y2 - cropBox.y1) > 0.15) {
        actionPrompt.innerText = "Pinch to capture";
        actionPrompt.classList.add('visible');
      } else {
        actionPrompt.innerText = "Spread hands to frame";
        actionPrompt.classList.add('visible');
      }
    } else {
...
// Convert normalized camera coords to local puzzle coords
...
function drawCameraOverlay() {
  ...
  if (mode === 'calibrate') {
    ...
    cameraCtx.fillRect(viewX1, y1 - 26, 90, 26);
    cameraCtx.fillStyle = '#f3f4f6';
    cameraCtx.font = '12px Outfit';
    cameraCtx.fillText('CROP AREA', viewX1 + 8, y1 - 8);
  }
...
// Capture webcam frame and create puzzle
...
// Initialize puzzle from image canvas
...
function checkSolved() {
  ...
  if (isWin) {
    ...
    setTimeout(() => {
      ...
      victoryGrid.innerText = `${gridSize}x${gridSize}`;
...
// Reset game (called from Play Again)
function resetGame() {
  playSound('click');
  mode = 'calibrate';
  tiles = [];
